...
Q(\theta^t, \theta^{t-1}) &= E_{P(Z|X, \lambda^{t-1}, \pi^{t-1})}[log(P(X,Z|\lambda,\pi))] \\
  ...
  + \sum_{i:x_i \ge 0}^{N} \big[\log(1- \pi) + x_i \log \lambda -\lambda - \log x_i! \big] \numberthis \label{e:Q}
\end{align*}
The update rule can be obtained by finding the derivative of \ref{e:loglike} w.r.t $\lambda$ and $\pi$.
\begin{align*}
	\frac{\partial logL}{\partial \lambda} & = \sum_{i:x_i=0}^{N} (z_i-1) + \sum_{i:x_i \ge 0}^{N} [\frac{x_i}{\lambda} -1 ] \numberthis \\
	\frac{\partial logL}{\partial \pi} &= \sum_{i:x_i=0}^{N} (\frac{z_i}{\pi} - \frac{(1-z_i)}{(1-\pi)}) + \sum_{i:x_i \ge 0}^{N} \frac{-1}{1-\pi}
...
\subsubsection{}
\begin{enumerate}
	\item The circles dataset is not linearly separable. K means doesnt have right objective function to optimize. 
